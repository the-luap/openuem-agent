...
func NewMSI(t *testing.T, directory string) MSI {
	t.Helper()
	return newMSI(t, directory, nil)
}

// NewMSIMajorUpgrade relates only to a family generated by this fixture. No
// caller-supplied upgrade code can select an existing product on the runner.
func NewMSIMajorUpgrade(t *testing.T, directory string, previous MSI) MSI {
	t.Helper()
	if previous.upgradeCode == "" || previous.Version != "1.2.3" {
		t.Fatal("owned major upgrade requires a generated first-version fixture")
	}
	return newMSI(t, directory, &previous)
}

func newMSI(t *testing.T, directory string, previous *MSI) MSI {
	t.Helper()
	...
	f.upgradeCode = guid()
	if previous != nil {
		f.upgradeCode, f.Version = previous.upgradeCode, "1.2.4"
	}
	...
	properties := map[string]string{"ProductCode": f.ProductCode, "ProductVersion": f.Version, "ProductName": "OpenUEM owned MSI fixture", "ProductLanguage": "1033", "Manufacturer": "OpenUEM owned tests", "ALLUSERS": "1", "ARPSYSTEMCOMPONENT": "1", "INSTALLLEVEL": "1", "UpgradeCode": f.upgradeCode}
	keys := make([]string, 0, len(f.Properties))
	for key := range f.Properties {
		keys = append(keys, key)
	}
	if previous != nil {
		keys = append(keys, "OWNEDUPGRADE")
		// Match only the exact first version in this randomly generated family.
		execute("CREATE TABLE `Upgrade` (`UpgradeCode` CHAR(38) NOT NULL, `VersionMin` CHAR(20), `VersionMax` CHAR(20), `Language` CHAR(255), `Attributes` SHORT NOT NULL, `Remove` CHAR(255), `ActionProperty` CHAR(72) NOT NULL PRIMARY KEY `UpgradeCode`, `VersionMin`, `VersionMax`, `Language`, `Attributes`)")
		execute(fmt.Sprintf("INSERT INTO `Upgrade` (`UpgradeCode`, `VersionMin`, `VersionMax`, `Language`, `Attributes`, `ActionProperty`) VALUES ('%s', '%s', '%s', '1033', 768, 'OWNEDUPGRADE')", f.upgradeCode, previous.Version, previous.Version))
		execute("INSERT INTO `InstallExecuteSequence` (`Action`, `Sequence`) VALUES ('FindRelatedProducts', 25)")
		// Join replacement to the installation transaction, before script generation.
		execute("INSERT INTO `InstallExecuteSequence` (`Action`, `Sequence`) VALUES ('RemoveExistingProducts', 1550)")
	}
	sort.Strings(keys)
