...
docker run --rm --init --cidfile "$unit_test_dir/container-id" --network none --read-only \
  ...
  "$unit_test_image" sh -eu -c '
    ...
    [ "$result" -eq 0 ]
    for test in TestLinuxUnitAcceptedByNativeSystemdParser TestLinuxManagerPrivateAuthenticationAndCalls TestLinuxManagerRejectsPIDBeforeAuthentication TestLinuxManagerRejectsUnsafeAndChangedNamespace TestLinuxManagerBoundsAuthenticationAndSanitizesErrors TestLinuxManagerCancellationAndJoinedClose TestLinuxManagerAuthenticatesKernelUID TestLinuxUnitFilePublicationAndRetainedRetry TestLinuxUnitFileRejectsForeignDefinitionsAndMetadata TestLinuxUnitFilePreservesChangedNamespaceAndCancellation TestLinuxUnitFileConcurrentExclusivePublication TestLinuxLoadedUnitObservationUsesTypedReadOnlyCalls TestLinuxDefinitionResolvesBeforeAdmittingAbsence TestLinuxEnablementRetainsCanonicalLinkWithoutMutation TestLinuxEnablementRejectsForeignLinksAndParents TestLinuxEnablementRejectsReplacementsAndJoinsClose TestLinuxManagerWaitsForAuthenticationConsumption TestLinuxRegistrationAdmitsOnlyOwnedPersistentState TestLinuxRegistrationSerializesAndJoinsCancellation TestLinuxStartRequiresStableSignedProcessReadiness TestLinuxStartCancellationJoinsManagerAndReadiness TestLinuxStartJobRequiresCanonicalTypedIdentity TestLinuxConfigurationPublishesAndPreservesMarkedSettings TestLinuxConfigurationRejectsForeignFilesAndNamespaces TestLinuxConfigurationRetainsReplacementsAndCancellation TestLinuxConfigurationConcurrentExclusivePublication TestLinuxManagerJoinsTruncatedMessageAlignment; do
